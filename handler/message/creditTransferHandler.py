...
def requestMessage(requestData):
    ...
    print(f"PAYMENT_TYPE: {paymentData.creditTransfer.get('PAYMENT_TYPE')}")
    generatedBizMsgIdr = handler.generateBizMsgIdr(generalData.sampleData.get(
        'DBTRAGT'), paymentData.creditTransfer.get('PAYMENT_TYPE'))
    generatedMsgId = handler.generateMsgId(generalData.sampleData.get(
        'DBTRAGT'), paymentData.creditTransfer.get('PAYMENT_TYPE'))
    ...
    response = requests.post(
        f"{SCHEME_VALUE}{generalData.sampleData.get('HOST_URL')}:{generalData.sampleData.get('HOST_PORT')}", json=filled_data, headers=headers)
