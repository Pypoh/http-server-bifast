...
from config.serverConfig import SCHEME_VALUE, HOST_URL_VALUE, PORT_VALUE
...
def requestMessage():
    ...
    with open(filePath, 'r') as file:
        ...
        value_dict = {
            ...
            "BIZ_MSG_IDR_VALUE": handler.generateBizMsgIdr("510"),
            "CRE_DT_VALUE": handler.getCreDt(),
            "MSG_ID_VALUE": handler.generateMsgId(bizMsgIdr),
            "CRE_DT_TM_VALUE": handler.getCreDtTm(),
            "END_TO_END_ID_VALUE": handler.generateBizMsgIdr("510"),
            "TX_ID_VALUE": handler.generateMsgId(bizMsgIdr),
    ...
    response = requests.post(f"{SCHEME_VALUE}{HOST_URL_VALUE}:{PORT_VALUE}", json=jsonResponse)
